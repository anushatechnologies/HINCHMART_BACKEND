...
export const createReview = async (req: Request, res: Response) => {
  try {
    ...
    if (rating >= 4) { sentiment = 'POSITIVE'; sentimentScore = 0.8 + (Math.random() * 0.2); }
    else if (rating <= 2) { sentiment = 'NEGATIVE'; sentimentScore = 0.1 + (Math.random() * 0.2); }
    else { sentimentScore = 0.4 + (Math.random() * 0.2); }
