...
export const getDashboardHome = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    // 3. Top Products
    const productSales: Record<number, { name: string; sales: number; revenue: number; stock: number }> = {};
    orderItems.forEach(item => {
...
export const getSalesDashboard = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    orderItems.forEach(item => {
      ...
      // Channels
      if (item.order.isCreditPurchase) creditRev += rev;
      else if (item.order.user.companyName) b2bRev += rev;
      else marketRev += rev;
