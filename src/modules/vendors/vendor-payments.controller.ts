...
export const getPayoutsOverview = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    stats.forEach(stat => {
      if (stat.status === 'PENDING') nextPayout = Number(stat._sum.netAmount) || 0;
      if (stat.status === 'PAID') totalSettled = Number(stat._sum.netAmount) || 0;
    });
...
export const getSettlementsLedger = async (req: Request, res: Response): Promise<void> => {
  try {
    const vendorId = parseInt(req.query.vendorId as string, 10);
    if (isNaN(vendorId)) {
      res.status(400).json({ success: false, message: 'vendorId is required' });
...
export const linkRazorpayAccount = async (req: Request, res: Response): Promise<void> => {
  try {
...
export const requestManualPayout = async (req: Request, res: Response): Promise<void> => {
  try {
    const vendorId = parseInt(req.params.id, 10);
