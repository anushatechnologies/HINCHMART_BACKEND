 ...
 */
export const requestNewBrand = async (req: Request, res: Response): Promise<void> => {
  try {
    const vendorId = parseInt(req.params.id, 10);
    if (isNaN(vendorId)) {
      res.status(400).json({ success: false, message: 'Invalid vendor ID' });
    ...
    const {
      ...
      website,
      brandDocuments,
      authorizationDocumentUrl, // can also be uploaded if they are not the sole owner but are adding a new brand, though usually Option B is OWNER
      brandDocumentUrl,
      gstin
    } = req.body;
 ...
 */
export const requestBrandAccess = async (req: Request, res: Response): Promise<void> => {
    try {
      const vendorId = parseInt(req.params.id, 10);
      if (isNaN(vendorId)) {
        res.status(400).json({ success: false, message: 'Invalid vendor ID' });
      ...
      const {
        brandId,
        accessType, // DISTRIBUTOR, DEALER, RESELLER
        authorizationDocumentUrl,
