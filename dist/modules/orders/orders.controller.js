...
exports.verifyRazorpayPayment = exports.createRazorpayOrder = exports.getOrderInvoice = exports.getMyOrders = exports.checkout = void 0;
...
const checkout = async (req, res) => {
    try {
        ...
        const address = await prisma_1.default.address.findUnique({ where: { id: parseInt(addressId) } });
        if (!address) {
        ...
        if (cart.user?.companyId) {
            contracts = await prisma_1.default.companyContract.findMany({
        ...
        cart.items.forEach(item => {
            ...
            if (contracts.length > 0) {
                ...
                if (contract)
                    currentPrice = parseFloat(contract.customPrice.toString());
            }
            subtotal += (currentPrice * item.quantity);
            // We attach the active price to the item so we can save it later
            item.activePrice = currentPrice;
        });
        ...
        // Send Notification
        await notifications_1.NotificationService.sendOrderConfirmation(cart.user.email || '', cart.user.phone || 'N/A', order.orderNumber, total);
      ...
      <html>
        ...
        <body>
          ...
          <table class="totals">
            <tr>
              <th>Subtotal:</th>
              <td>₹${parseFloat(order.subtotal).toLocaleString('en-IN')}</td>
            </tr>
            ${parseFloat(order.cgst) > 0 ? `
              <tr>
                <th>CGST (9%):</th>
                <td>₹${parseFloat(order.cgst).toLocaleString('en-IN')}</td>
              </tr>
              <tr>
                <th>SGST (9%):</th>
                <td>₹${parseFloat(order.sgst).toLocaleString('en-IN')}</td>
              </tr>
            ` : ''}
            ${parseFloat(order.igst) > 0 ? `
              <tr>
                <th>IGST (18%):</th>
                <td>₹${parseFloat(order.igst).toLocaleString('en-IN')}</td>
              </tr>
            ` : ''}
            <tr>
              <th>Shipping Fee:</th>
              <td>₹${parseFloat(order.shippingFee).toLocaleString('en-IN')}</td>
            </tr>
...
const verifyRazorpayPayment = async (req, res) => {
    try {
        ...
        // 2. Fetch the order
        const order = await prisma_1.default.order.findUnique({ where: { id: parseInt(orderId) } });
        if (!order) {
            return res.status(404).json({ success: false, message: 'Order not found' });
        }
