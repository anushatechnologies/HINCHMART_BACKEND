...
exports.granularSectionReview = exports.submitKycForReview = exports.saveOnboardingStep = void 0;
...
const saveOnboardingStep = async (req, res) => {
    try {
        ...
        const currentVendor = await prisma_1.default.vendor.findUnique({ where: { id: Number(vendorId) } });
        if (!currentVendor) {
 ...
 */
const submitKycForReview = async (req, res) => {
    try {
...
exports.submitKycForReview = submitKycForReview;
/**
 * Granular Section Review by Admin
 * PATCH /api/vendors/:id/granular-review
 */
const granularSectionReview = async (req, res) => {
    try {
        const { id } = req.params;
        const { section, status, rejectionReason } = req.body;
        // section: 'businessInfo' | 'gst' | 'pan' | 'bank' | 'warehouse' | 'docs'
        // status: 'APPROVED' | 'REJECTED'
        const vendor = await prisma_1.default.vendor.findUnique({ where: { id: Number(id) } });
        if (!vendor) {
            res.status(404).json({ success: false, message: 'Vendor not found' });
            return;
        }
        let existingReasons = vendor.sectionRejectionReasons || {};
        ...
        const allApproved = updatedVendorState.businessInfoStatus === 'APPROVED' &&
        ...
        const anyRejected = updatedVendorState.businessInfoStatus === 'REJECTED' ||
        ...
        const finalVendor = await prisma_1.default.vendor.update({
            where: { id: Number(id) },
            data: updatePayload
        });
        res.status(200).json({
...
exports.granularSectionReview = granularSectionReview;
